	...
	*/
	...
	async function coffeeParty() {
		for (const city of cities) {
			...
			if (office.avgEnergy < 95) c.buyTea(agricultureName, city)
			if (office.avgMorale < 95) c.throwParty(agricultureName, city, 500_000)
		}
	}
	...
	async function waitForTheLazyFucksToGetTheirShitTogether() {
		...
		ns.print("   avg morale: " + (avgs[0] / 6).toFixed(3) + "/95")
		ns.print("   avg energy: " + (avgs[1] / 6).toFixed(3) + "/95")
		stage[1]++;
		if (avgs[0] / 6 >= 95 && avgs[1] / 6 >= 95 && stage[1] > 0) { stage[0] += 1; stage[1] = 0; }
	}

	//Reassing the employees so that nobody works in R&D
	async function reAssignEmployees() {
		for (let city of cities) {
            try {c.setAutoJobAssignment(agricultureName, city, jobs[4], 0)} catch(_){}
            try {c.setAutoJobAssignment(agricultureName, city, jobs[0], 3)} catch(_){}
            try {c.setAutoJobAssignment(agricultureName, city, jobs[1], 2)} catch(_){}
            try {c.setAutoJobAssignment(agricultureName, city, jobs[2], 2)} catch(_){}
            try {c.setAutoJobAssignment(agricultureName, city, jobs[3], 2)} catch(_){}
		}
		stage[0]++;
		stage[1]=0;
	}
	...
	async function invest(i) {
		...
		if (stage[1] <= 10) {
			ns.print("waiting cycles: " + stage[1] + "/10. investors are currently offering: " + ns.nFormat(c.getInvestmentOffer().funds, "0.00a"));
			stage[1] += 1;
		}
		else {
			ns.tprint("investment offer round " + i + ": " + ns.formatNumber(c.getInvestmentOffer().funds, "0.00a"))
		...
		for (let i = 0; i < 2; i++) {
			for (let city of cities) {
				try {
					...
					try {c.setAutoJobAssignment(agricultureName, city, jobs[0], 1)} catch(_){}
					try {c.setAutoJobAssignment(agricultureName, city, jobs[1], 1)} catch(_){}
					try {c.setAutoJobAssignment(agricultureName, city, jobs[2], 1)} catch(_){}
					try {c.setAutoJobAssignment(agricultureName, city, jobs[3], 1)} catch(_){}
					try {c.setAutoJobAssignment(agricultureName, city, jobs[4], 5)} catch(_){}
				} catch { }
			}
		}
	...
	async function expandToTobacco() {
		...
		try {
			for (let i = 0; i < 9; i++) {
				...
				try {c.setAutoJobAssignment(tobaccoName, cities[0], jobs[0], Math.floor(c.getOffice(tobaccoName, cities[0]).numEmployees / 3.5))} catch(_){}
				try {c.setAutoJobAssignment(tobaccoName, cities[0], jobs[1], Math.floor(c.getOffice(tobaccoName, cities[0]).numEmployees / 3.5))} catch(_){}
				try {c.setAutoJobAssignment(tobaccoName, cities[0], jobs[2], Math.floor(0.5 * c.getOffice(tobaccoName, cities[0]).numEmployees / 3.5))} catch(_){}
				try {c.setAutoJobAssignment(tobaccoName, cities[0], jobs[3], Math.ceil(c.getOffice(tobaccoName, cities[0]).numEmployees / 3.5))} catch(_){}
			}
		} catch { }
		...
		for (let i = 0; i < 2; i++) {
			for (let city of cities) {
                ...
                try {c.setAutoJobAssignment(tobaccoName, city, jobs[0], 1)} catch(_){}
                try {c.setAutoJobAssignment(tobaccoName, city, jobs[1], 1)} catch(_){}
                try {c.setAutoJobAssignment(tobaccoName, city, jobs[2], 1)} catch(_){}
                try {c.setAutoJobAssignment(tobaccoName, city, jobs[3], 1)} catch(_){}
                try {c.setAutoJobAssignment(tobaccoName, city, jobs[4], 5)} catch(_){}
			}
		}
